...
function addTask () {
    ...
    const taskData = {
        ...
        id: Date.now(), // выдает время в милисекундах
        text: inputTaskValue,
        done: false
    }
...
function checkTask (event) {
    ...
    if (event.target.closest('.task__complite')) {
        let itemTask = event.target.closest('.task-app')
            ...
            const index = tasksArr.find((task) => task.id === id )
            
            index.done = !index.done;
            saveToLocalStorage();
            itemTask.classList.toggle('complite')
    }
}
...
function checkEmpty () { 
    if(tasksArr.length === 0) {
        const emptyTemplate = `
       <li class="list__empty empty">
            <div class="empty__title">Список задач пуст :(</div>
            <div class="empty__img"><img src="./assets/img/icons/empty.svg" alt="Список задач пуст"></div>
               </li>
        `
        taskList.insertAdjacentHTML('beforeend', emptyTemplate);
    } else {
...
function renderTaskHTML (item) {
    const cssClass = item.done ? 'app__task task-app  complite' : 'app__task task-app' // проверка на false // true выполнение задачи
